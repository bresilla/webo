...
class Tracktor(Node):
    def __init__(self, name, sub_name, pub_name_track, pub_name_yolo):
        super().__init__(name)
        ...
        self.image_pub = self.create_publisher(Image, pub_name_track, 10)
        self.yolo_pub = self.create_publisher(Image, pub_name_yolo, 10)
    ...
    def callback(self, image, distance):
        ...
        tags, image_clone = self.gen_bboxes(frame, image.header.stamp)
        ...
        image_clone = self.bridge.cv2_to_imgmsg(image_clone, encoding='bgr8')
        self.image_pub.publish(labeled_frame)
        self.yolo_pub.publish(image_clone)

    def gen_bboxes(self, img, img_header_stamp, min_area=1000):
